...
origin_bed_list = open(path + 'list_all_non_ChIP_origin.txt').readlines()
avail_files = []
...
for file_index in range(len(avail_files)):
	...
	for i in range(len(input)):
		...
		if bed_input_row[0] in chr_label:
			...
			diff = (right / 200) - (right // 200)
			if(i < (len(input) - 1)):
				if((input[i].split('\t')[0] == input[i+1].split('\t')[0]) & (diff > 0) & (diff < 0.5) & (int(input[i+1].split('\t')[1]) - right < 100)):
					right = right + 100
			if( (right - left) >= 100 ):
				if( ((left // 200) != (left / 200)) & ((left // 100) == (left / 100)) ):
					left = left - 100			
				if( ((right // 200) != (right / 200)) & ((right // 100) == (right / 100)) ):
					right = right - 100
				input_coordinates.append( round( left / 200.0 ) )
				input_coordinates.append( round( right / 200.0 ) )
