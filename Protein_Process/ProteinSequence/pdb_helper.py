...
def template_compare(dict_1, dict_2, order_1 = None, order_2 = None, chain = False, order = False, threshold=0.5):
    if order and (order_1 == None or order_2 == None):
        print('Error! Need to input the ordered chain list!')
        return None
    elif order and (order_1 != order_2):
        return False
    elif chain:
        if (not set(dict_1.keys()) == set(dict_2.keys())):
            return False
        else:
            for chain in dict_1.keys():
                if seq_similarity(dict_1[chain]['seq'],dict_2[chain]['seq']) < threshold:
                    return False
            return True
    else:
        selected_chain = []
        if len(dict_1.keys()) != len(dict_2.keys()):
            ...
            for chain in dict_1.keys():
                if chain in dict_2.keys() and seq_similarity(dict_1[chain]['seq'],dict_2[chain]['seq']) < threshold:
                    return False
                elif not (chain in dict_2.keys()):
                    temp = False
                    for chain_2 in dict_2.keys():
                        if not (chain_2 in dict_1.keys() or chain_2 in selected_chain)\
                        and seq_similarity(dict_1[chain]['seq'],dict_2[chain_2]['seq']) >= threshold:
                            temp = True
                            selected_chain.append(chain_2)
                            break
                    if not temp:
                        return False
            return True
