...
def uniChainMapping(working_dir,pdb_id, unpAcc_list):
  ...
  Output:
  1. uniprotChain_mapping_dict (mapping between uniprot ids and chain ids)
    ...
    if res_align.status_code != 200:
      ...
      for d in res_align_json['data']['alignment']['target_alignment']:
        if d['target_id'] == unpAcc_list[0]:
          ...
          chain_list1.append(chain_id)

        if d['target_id'] == unpAcc_list[1]:
          unp_seq=d['target_sequence']
          aligned_regions=d['aligned_regions']
          chain_list2.append(chain_id)

      if unp_seq is None: 
        # no such unpAcc under this pdb,
        pass
  # complete the mapping(dict)
  uniprotChain_mapping_dict[unpAcc_list[0]] = chain_list1
  uniprotChain_mapping_dict[unpAcc_list[1]] = chain_list2

  return uniprotChain_mapping_dict