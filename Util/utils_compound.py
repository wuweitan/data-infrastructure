...
def smiles2graph(smiles):
    """Convert smile sequence into graph.
    Args:
        smiles (str): Smiles sequence.
    Returns:  
        node feature (numpy.ndarray), adjacency matrix (numpy.ndarray): Node features and adjacency matrix.
    """
    try:
    ...
    adj_temp = adj_temp + np.eye(MAX_size)
