...
def read_graph(source):
    ...
    # return sps.csr_matrix(temp.reshape(1, -1)), sps.csr_matrix(adj_temp.reshape(1, -1))
    return temp.astype('float32'), adj_temp.astype('float32') # sps.csr_matrix(adj_temp.reshape(1, -1)).astype('int8'), source


'''
import multiprocessing as mp
# data = data.loc[[n for n in range(100000)]]
n_comp = len(data)
batch = 10000
n_iter = n_comp // batch + 1
stage_size = n_iter // 10

Vertex = []
Adj = []
n_stage = 9
# for n in tqdm(range(n_iter)):
for n in tqdm(range(n_stage*stage_size, (n_stage+1)*stage_size)):
    pool = mp.Pool(24)

    if n == (n_iter-1):
        out = pool.map(read_graph, [data.loc[_n].SMILES_string for _n in range(n*batch, n_comp)])
        break
    else:
        out = pool.map(read_graph, [data.loc[_n].SMILES_string for _n in range(n*batch, (n+1)*batch)])
    ...
    if (n+1) % stage_size == 0:
        n_stage = (n+1) // stage_size
        Vertex = sps.vstack(Vertex)
        Adj = sps.vstack(Adj)
        sps.save_npz('./processed_data/x_' + str(n_stage) + '.npz', Vertex)
        sps.save_npz('./processed_data/adj_' + str(n_stage) + '.npz', Adj)
        Vertex = []
        Adj = []
...
sps.save_npz('./processed_data/x_' + str(n_stage+1) + '.npz', Vertex)
sps.save_npz('./processed_data/adj_' + str(n_stage+1) + '.npz', Adj)

print(stage_size * n_stage + Vertex.shape[0])
'''
