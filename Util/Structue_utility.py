...
def remove(string, char):
    ...
    :type string: str.
    :param char: The characters to be removed.
    :type char: str.
    :returns:  str -- the processed string.
...
def read_pdb(pdb_file):
    ...
    :param pdb_file: The path of the PDB file.
    :type pdb_file: str.
    :returns: 
...
def pdb_truncate(pdb_file, chain, start, end, relative_start=False, fix_length=False):
    """Select a certain region of the protein in the pdb file.

    :param pdb_file: The path of the PDB file.
    :type pdb_file: str.
    :param chain: The chain to be queried.
    :type chain: str.
    :param start, end: start point and end point for the target region.
    :type start, end: int.
    :returns:  list -- a list of the selected lines in the PDB file.
    :raises: AttributeError, KeyError
...
#################################################################################
# 1-D Features: Sequences
#################################################################################

#**************************** AA sequences **************************************

def seq_extraction(pdb, standard_pdb = True):
    """Extract the amino acid sequences of all the chains in a pdb file with corresponding information.

    :param pdb: The path of the PDB file.
    :type pdb: str.
    :param standard_pdb: whether the file is under the standard PDB format.
    :type standard_pdb: bool.
    :returns: dict -- a dictionary with the chain indices as the keys and the amino acid sequences, sequence length, residue range and missing residue indices as the contents.
    :raises: AttributeError, KeyError
    ...
    if seq != '':
        seqs.append({'model':model, 'chain': chain_pre, 'seq': seq, 'length': len(seq), 'resi_range':[start, index_all_pre], 'miss_resi':missing, 'insertion_num':inser_num})
    ### arrange the list into a dictionary
    result = {}
    for s in seqs:
        model = s['model']
        chain = s['chain']
...
def read_pdb_ss(header_file, chain_ref):
    ...
    :param header_file: The path of the PDB file (with the header) or the PDB Header file.
    :type header_file: str.
    :param chain_ref: the chain to be queied.
    :type chain_ref: str
    :returns: dict -- a dictionary tell the region of the helices and the strands of the target chain (the other parts are coils).
    :raises: AttributeError, KeyError
...
def rcsb_api_ss(pdb, chain, PDB_chain = True):
    ...
    :param pdb: PDB ID.
    :type pdb: str.
    :param chain: the chain to be queied.
    :type chain: str
    :param PDB_chain: whether apply the PDB chain indices (or author defined chain indices).
    :type PDB_chain: bool
    :returns: dict -- a dictionary tell the region of the helices and the strands of the target chain (the other parts are coils).
    :raises: AttributeError, KeyError
...
def to_nxGragh(A, X=None, Y=None):
    ...
    :type A: torch.Tensor or numpy.ndarray.
...
def to_dglGraph(A, version = 'Homogeneous', edge_type = None):
    ...
    :param A: adjacency tensor.
    :type A: torch.Tensor or numpy.ndarray.
    :param version: versions of the graphs, "Homogeneous" or "Heterogenous". Default: "Homogeneous"
    :type version: str.
    :param edge_type: list of the edge types.
    :type edge_type: str.
    :returns:  DGL.graph 
    :raises: AttributeError, KeyError
...
def TMscore_sym(pdb_1, pdb_2):
    ...
    :param pdb_1, pdb_2: path of the pdb files
    :type pdb_1, pdb_2: str
    :returns: float --symmetric TM-score
    '''
    command_1 = './TMalign ' + pdb_1 + ' ' + pdb_2 + ' -a'
    output_1 = os.popen(command_1)
    out_1 = output_1.read()
